...
import {
  Tree, Modal, Button, Row, Col, Icon, Input
} from 'antd';
...
export default class TreeComponent extends React.Component {
  ...
  renderTreeNode(node) {
    const {
      type, name, uuid, isEmpty, fileType
    } = node;
    if (type === 'directory') {
      return (
        <TreeNode
  ...
  renderTreeNodes = lists => lists.map((node, index) => {
    const {
      isEmpty, children, name, uuid
    } = node;
    if (!isEmpty && children.length) {
      return (
          <TreeNode
  ...
  render() {
    ...
    return (
      <Modal
        ...
        width={600}
      >
        <Tree showLine showIcon loadData={this._onLoadData} onSelect={this.onSelect}>
          <TreeNode
            icon={<FileIcon type="folder" />}
            title={'全部文件'}
            key={'1024'}
        ...
        {this.isSave ? (
          ...
          <Row type="flex" justify="end" style={{ marginTop: 20 }}>
            ...
            <Col>
              <Button style={{ marginRight: 0, padding: '0px 35px' }} onClick={this._onCancel}>
                取消
              </Button>
            </Col>
          </Row>
